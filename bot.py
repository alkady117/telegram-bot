...
def change_password(link, new_password):
    """تغيير كلمة المرور - نسخة محسنة تتحقق من النجاح"""
    try:
        # استخراج uidb36 و token من الرابط
        try:
        ...
        enc_password = f"#PWD_INSTAGRAM:0:{ts}:{new_password}"
        ...
        # التحقق من النجاح الحقيقي
        if "status" in r.text and '"status":"ok"' in r.text:
